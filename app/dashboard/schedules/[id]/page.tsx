...
export default function ScheduleDetailPage({
  ...
  const fetchScheduleData = async () => {
    setPageLoading(true);
    try {
      const [scheduleRes, companiesRes, busesRes, routesRes] =
        await Promise.all([
      ...
      if (scheduleRes.ok) {
        ...
        const companyId = scheduleData.company
          ? typeof scheduleData.company === "object"
            ? scheduleData.company._id
            : scheduleData.company
          : null;
        const busId = scheduleData.bus
          ? typeof scheduleData.bus === "object"
            ? scheduleData.bus._id
            : scheduleData.bus
          : null;
        const routeId = scheduleData.route
          ? typeof scheduleData.route === "object"
      ...
      if (selectedBusId) {
        try {
          ...
          if (res.ok) {
            ...
            // If schedule has seats, use those instead
            const scheduleSeats = form.getFieldValue("seats") || [];
            if (Array.isArray(scheduleSeats) && scheduleSeats.length > 0) {
              busSeatsData = scheduleSeats;
            }
            ...
            busSeatsData = busSeatsData.map((seat: any) => ({
              ...
              fare: seat.fare ? Number(seat.fare) : 0,
            }));

            // Process rows, columns, aisleColumns with proper fallbacks
  ...
  if (!schedule) {
    return (
      <div className="max-w-7xl mx-auto">
        <div className="text-center py-12">Schedule not found</div>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Update trip schedule with bus, route, and timing details
        </p>
      </div>
